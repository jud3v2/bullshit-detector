...
create table if not exists public.usage_windows (
  id uuid primary key default gen_random_uuid(),
  user_id uuid not null references auth.users(id) on delete cascade,
  window_type text not null check (window_type in ('hour', 'week', 'month')),
...
insert into public.subscription_products (
  ...
  ('free', 'Free', 'Manual analysis plus 3 AI analyses per month.', 0, 0, 3, 3, 3),
  ('starter', 'Starter', 'Premium AI analysis with controlled weekly credits.', 299, 100, 12, 80, 320),
  ('plus', 'Plus', 'More AI credits and richer analysis history.', 499, 250, 30, 220, 880),
  ('pro', 'Pro', 'High-limit professional usage.', 2000, 2000, 120, 1600, 6400)
