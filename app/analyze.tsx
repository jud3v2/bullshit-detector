...
function formatLimitNotice(limit: LimitCheck, language: Language) {
  if (language === 'fr') {
    if (limit.reason === 'subscription_required') {
      if (limit.plan.id === 'free') {
        return `Limite gratuite atteinte: 3 analyses IA par mois. Reset: ${formatResetDate(limit.nextMonthlyResetAt, language)}.`;
      }
    ...
    if (limit.reason === 'monthly_analysis_limit') {
      return `Limite mensuelle atteinte. Reset: ${formatResetDate(limit.nextMonthlyResetAt, language)}.`;
    }

    if (limit.reason === 'hourly_limit') {
      return `Limite horaire atteinte. Reset: ${formatResetDate(limit.nextHourlyResetAt, language)}.`;
    }

    if (limit.reason === 'weekly_budget_limit') {
      return `Credits hebdomadaires atteints. Reset: ${formatResetDate(limit.nextWeeklyResetAt, language)}.`;
    }

    return `Limite hebdomadaire atteinte. Reset: ${formatResetDate(limit.nextWeeklyResetAt, language)}.`;
  }

  if (limit.reason === 'subscription_required') {
    if (limit.plan.id === 'free') {
      return `Free limit reached: 3 AI analyses per month. Reset: ${formatResetDate(limit.nextMonthlyResetAt, language)}.`;
    }
  ...
  if (limit.reason === 'monthly_analysis_limit') {
    return `Monthly limit reached. Reset: ${formatResetDate(limit.nextMonthlyResetAt, language)}.`;
  }

  if (limit.reason === 'hourly_limit') {
    return `Hourly limit reached. Reset: ${formatResetDate(limit.nextHourlyResetAt, language)}.`;
  }

  if (limit.reason === 'weekly_budget_limit') {
    return `Weekly credits reached. Reset: ${formatResetDate(limit.nextWeeklyResetAt, language)}.`;
  }

  return `Weekly limit reached. Reset: ${formatResetDate(limit.nextWeeklyResetAt, language)}.`;
}
                ...
                <View
                  ...
                  <View style={styles.socialBody}>
                    ...
                    {connected && session.expiresAt ? (
                      <Text style={[styles.socialMeta, { color: palette.muted }]}>
                        {s.sessionWindow}: {formatHumanDate(session.expiresAt, language)}
                      </Text>
                    ) : null}
                  </View>
                  <Pressable
          ...
          <View style={styles.header}>
            <View style={styles.topRow}>
              <Pressable
                accessibilityRole="button"
                onPress={() => router.push('/')}
                style={[styles.privacyPill, { backgroundColor: palette.ink }]}>
                <MaterialCommunityIcons name="view-dashboard-outline" size={14} color={palette.accent} />
                <Text style={[styles.privacyText, { color: palette.inkText }]}>
